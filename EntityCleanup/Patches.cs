...
namespace EntityCleanup
{
	...
	class PickupPatch
	{
		public static bool Prefix(ref bool __result, Inventory inv, InventorySystem.Items.ItemBase item, InventorySystem.Items.Pickups.PickupSyncInfo psi, bool spawn = true)
		{
			if (!NetworkServer.active)
			{
				throw new InvalidOperationException("Method ServerCreatePickup can only be executed on the server.");
			}
			InventorySystem.Items.Pickups.ItemPickupBase itemPickupBase = GameObject.Instantiate<InventorySystem.Items.Pickups.ItemPickupBase>(item.PickupDropModel, inv.transform.position, global::ReferenceHub.GetHub(inv.gameObject).PlayerCameraReference.rotation * item.PickupDropModel.transform.rotation);
